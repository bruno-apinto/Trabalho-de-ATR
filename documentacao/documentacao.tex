...
Como requisitado pelo enunciado do trabalho, é necessário implementar pelo menos uma comunicação interprocesso (IPC) e implementar signal entre duas tarefas, para a utilização da câmera.

As tarefas \textit{comando\_navegacao} e \textit{controle\_navegacao} foram implementadas em processos diferentes. A troca de informações entre eles é feita por meio da Memória Compartilhada, implementada na própria main. A Memória Compartilhada foi implementad apor meio da Biblioteca Boost, que cria um FileDescriptor para troca de informações. Essa abordagem adaptada foi necessária para haver compatibilidade com o sistema MacOS. Porém, a utilização se mantém da mesma forma. Já a utilização do signal foi utilizado entre para comunicar a presença de uma falha no túnel. Em \textit{reconstrucao\_teto}, ao identificar uma variação acima da esperada, o signal \texttt{SIGUSR1} é acionado. A função \textit{signal\_handler} trata esse signal, desacelerando o carrinho e acionando a câmera. 
